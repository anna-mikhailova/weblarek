...
```
interface IBuyer {
  payment: "card" | "cash"  | undefined; // Способ оплаты (только два допустимых значения)
...
Конструктор:  
`constructor(events: IEvents)` - создает пустой каталог товаров, принимает брокер событий.
...
Конструктор:  
`constructor(events: IEvents)` - создает пустую корзину, принимает брокер событий.
...
Конструктор:  
`constructor(events: IEvents)` - создает объект с пустыми данными покупателя, принимает брокер событий.  
...
Конструктор:  
`constructor(baseApi: IApi, private events: IEvents)` - принимает экземпляр класса, реализующего интерфейс IApi и брокер событий.
